...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const isPassworValid = await user.isPasswordCorrect(password)   //isPasswordCorrect is defined in userSchema in mongoose it compares the password using bcrypt, defined in user Schema check
    if (!isPassworValid) {
        throw new ApiError(40, "Password is wrong")
    }
    const { accessToken, refreshToken } = await generateAccessAndRefreshToken(user._id);

    const loggedInUser = await User.findById(user._id).select("-password -refreshToken") //Find the data in database of register user to give the cridentials to the login user
