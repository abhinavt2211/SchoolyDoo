...
function UpdateVideo() {
    ...
    const getVideo = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/v1/videos/fetchVideo/${videoID}`,
          {
    ...
    if (page === 1) {
      ...
      try {
        ...
        const res = await axios.patch(
          `http://localhost:8000/api/v1/videos/updateVideo/${vid?._id}`,
          {
  ...
  const handleTogglePublic = async () => {
    try {
      const res = await axios.patch(
        `http://localhost:8000/api/v1/videos/togglePublish/${vid?._id}`,
        {},
        {
          withCredentials: true,
        }
    ...
    try {
      ...
      if (confirm) {
        const res = await axios.delete(
          `http://localhost:8000/api/v1/videos/delete/${vid?._id}`,
          {
            withCredentials: true,
          }
        );
