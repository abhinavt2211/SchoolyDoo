...
function VideoPage() {
    ...
    const getVideo = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8000/api/v1/videos/fetchVideo/${videoID}`,
          {
  ...
  useEffect(() => {
    const getLikeDetails = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8000/api/v1/likes/likeStats/${videoID}`,
          {
            withCredentials: true,
          }
        );
  ...
  useEffect(() => {
    const getUserInfo = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8000/api/v1/users/get-user-channel/${video?.owner[0].username}`,
          {
            withCredentials: true,
          }
        );
  ...
  const handleSubscribe = async () => {
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/subscription/toggleSubscribe/${subInfo?._id}`,
        {},
        {
          withCredentials: true,
        }
  ...
  const handleLike = async () => {
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/likes/videoLiked/${videoID}`,
        {},
        {
          withCredentials: true,
        }
